...
$document.ready(function () {
  ...
  if (plugins.modal.length) {
    plugins.modal.on('hidden.bs.modal', function () {
      for (var i = 0; i < plugins.modal.length; i++) {
        var thisModal = $(plugins.modal[i]),
          thisModalIframe = thisModal.find('iframe');
        thisModalIframe.attr('src', thisModalIframe.attr('src'));
      }
    });
  }
});
